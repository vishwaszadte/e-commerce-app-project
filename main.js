    ...
    userModel
      ...
      .then(function (user) {
        if (user && !user.isVerifiedMail) {
          res.render("login", {
            error: "please check your mail and verify your account",
          });
          return;
        }
        if (user) {
  ...
  .post(upload.single("profile_pic"), function (req, res) {
    ...
    userModel
      .create({
        ...
        profile_pic: file
          ? file.filename
          : "profile_pic-1649695341010-454886408",
...
app.post("/changePassword", function (req, res) {
  ...
  userModel.findOne({ username: username }).then(async function (user) {
    if (user) {
      ...
      res.send(
        "password changed successfully, now you can login with your new pasword"
      );
    } else {
...
app.route("/addToCart").post(function (req, res) {
  ...
  if (!req.session.isLoggedIn) {
    res
      .status(401)
      .json({ status: false, message: "please login", data: null });
    return;
  }
...
app.route("/updateCart").post(function (req, res) {
  ...
  // res.send();
  cartModel
    .findOne({ user_id: user._id }, { product_id, product_id })
    .then(function (item) {
